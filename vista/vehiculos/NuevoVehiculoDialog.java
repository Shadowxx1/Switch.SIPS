...
//                if (vehiculo.getSerialCarroceria() != null && !vehiculo.getSerialCarroceria().trim().isEmpty()) {
//                    if (p) {
//                        where += " OR ";
//                    } else {
//                        where += " WHERE ";
//                    }
//                    where += "P.serialCarroceria=?";
//                    c = true;
//                }
                Query q = s.createQuery("SELECT count(P) FROM " + Vehiculo.class.getName() + " as P " + where);
                int i = 0;
                if (p) {
                    q = q.setString(i, vehiculo.getPlaca());
                    i++;
                }
                if (c) {
                    q = q.setString(i, vehiculo.getSerialCarroceria());
                    i++;
                }
                ...
                if (cant == 0) {
                    ...
                    q = s.createQuery("FROM " + Vehiculo.class.getName() + " as P"
                            + where);
                    i = 0;
                    if (p) {
                        q = q.setString(i, vehiculo.getPlaca());
