    ...
    class PolizaDialogController extends FormController {
        ...
        public Response insertRecord(ValueObject newPersistentObject) throws Exception {
            ...
            try {
                ...
                Long cant = (Long) s.createQuery("SELECT count(P) FROM " + Recibo.class.getName() + " as P WHERE P.numero=? AND P.poliza.numero=? AND P.poliza.compania.id=? AND P.poliza.ramoPoliza.id=?").
                        setString(0, recibo.getNumero()).
                        setString(1, recibo.getPoliza().getNumero()).
                        setLong(2, recibo.getPoliza().getCompania().getId()).
                        setLong(3, poliza2.getRamoPoliza().getId()).
                        list().get(0);
                if (cant == 0) {
                    if (poliza2 == null) {
                    ...
                    ok = true;
                    nuevo = false;
                    recibo = (Recibo) s.createQuery("SELECT P FROM " + Recibo.class.getName() + " as P WHERE P.numero=? AND P.poliza.numero=? AND P.poliza.compania.id=? AND P.poliza.ramoPoliza.id=?").
                            setString(0, recibo.getNumero()).
                            setString(1, recibo.getPoliza().getNumero()).
                            setLong(2, recibo.getPoliza().getCompania().getId()).
                            setLong(3, poliza2.getRamoPoliza().getId()).
